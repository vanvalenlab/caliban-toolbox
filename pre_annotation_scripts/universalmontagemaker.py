...
def maker():
    ...
    for cell_type, channel_name in zip(cell_types, channel_names):
        for set in sets_lst:
            direc = os.path.join(base_direc, cell_type, set)
            ...
            for i in range(segmenter):
                for j in range(segmenter):
                    ...
                    for stack_number in range(number_of_images):
                        ...
                        cropped_image_name = 'set_' + str(set[-1]) + '_x_' + str(i) + '_y_' + str(j) + '_slice_' + str(stack_number) + '.png'
                        cropped_folder_name = os.path.join(direc, save_stack_subdirec, 'set_' + str(set[-1]) + '_x_' + str(i) + '_y_' + str(j))
                    ...
                    montage_maker(time_segments, number_of_frames, direc, list_of_cropped_images, i, j, crop_size_x, set)



def montage_maker(time_segments, number_of_frames, direc, cropped_images, x_seg, y_seg, crop_size_x, set):
    ...
    if time_segments == 1:
        ...
        os.chdir('./montages/')
        save_direc_list.append('./montages/' + set)
