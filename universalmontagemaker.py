...
def maker():
    ...
    number_of_frames = 45
    cell_types = ['S3']
    list_of_number_of_sets = [8]
    channel_names = ['nuclear']
    segmenter = 5
    base_direc = '/data/data/cells/HeLa/'
        ...
        for set_number in range(number_of_sets):
            ...
            for i in range(segmenter):
                for j in range(segmenter):
                    ...
                    montage_maker(time_segments, number_of_frames, direc, list_of_cropped_images, i, j, crop_size_x, set_number)



def montage_maker(time_segments, number_of_frames, direc, cropped_images, x_seg, y_seg, crop_size_x, set_num):
    ...
    save_direc_list = []
    if time_segments == 1:
        if not os.path.isdir('./montages'):
                os.makedirs('./montages')
        os.chdir('./montages/')
        save_direc_list.append('./montages/set' + str(set_num))
        if not os.path.isdir('./set' + str(set_num)):
            os.makedirs('./set' + str(set_num))
        os.chdir('../')

        #save_direc_list = ['./montages/set0', './montages/set1', './montages/set2', './montages/set3', './montages/set4', './montages/set5', './montages/set6', './montages/set7']
